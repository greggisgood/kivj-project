...
public class PlayerSkeleton {
	...
	// Duplicate Fields from State, used in simulating of possible next move
	...
	protected int height; 		// Landing Height of piece
	protected int totalHeight;	// Sum of Heights of all columns
	protected int diffHeight;	// Sum of difference of adjacent columns
	protected int altitudeDiff;	// Difference between max and min column
	protected int maxHeight;	// Height of highest column
	protected int rowsCleared; 	// Rows cleared by a Single Move, not total rows cleared
	protected int weightedSum;	// Sum of number of filled cells, weighted by the height of the cell 
	protected int holeCount;	// Number of empty cells with at least 1 filled cell above in same column
	protected int connectedHole;// Number of holes, vertically connected holes are considered as 1
	protected int maxWellDepth; // Max depth of well (depression on the top surface of the blocks)
	protected int sumWells;		// Sum of depths of all wells
	protected int weightedWells; // Sum of depths of wells,weighted(depth 4 well has weight 1+2+3+4)
	protected int hTransition;   // Number of transitions(filled -> empty cell and vice versa) horizontally
	protected int vTransition;  // Number of transitions vertically
	protected int erodedCells; // rowsCleared * number of cells cleared belonging to this turn's piece
	
	// Best set of weights obtained from experiments
	protected static double[] getBestWeights()
	{
		double [] w = {11.708585390569464, 0.0, 1.8974325060818578, 0.0, 0.0, 100.0, 0.0, 83.40570775458364, 19.74020548619076, 49.12019352048655, 6.780747031944694, 35.93440126423564, 95.00893395888016, 6.4556750383596775, 19.070132593850886};
		return w;
	}
	
// Following code are taken from State.java, used in simulation possible next moves
	...
	// Copy of makeMove
	// Evaluates Feature 13 - rows cleared in this move
	// Evaluates Feature 14 - Eroded Cells in this move
...
// End of code duplicate from State.java
...
// Plays using the set of seeds in seeds.txt and writes score into results.txt
...
//	double [] w10 = {61.88706839048098, 0.0, 0.0, 0.0, 2.830323893727921, 100.0, 0.0, 100.0, 0.0, 9.394831701193526, 32.371113940085, 17.889372940018198, 80.93552071247848, 8.288340692659537, 0.0};
			...
			try {
				...
				while ((line = (bufferedReader.readLine())) != null) 
				{
					...
					try {
						...
						bufferedWriter.append("" + s.getRowsCleared() + '\n');
					} catch (Exception ex) {
						ex.printStackTrace();
					} finally {
	...
	public static void getRandomSeed(String[] args) {
		...
		//	s.setSeed(seed);
			...
			PlayerSkeleton p = new PlayerSkeleton(); 
			p.setWeights(PlayerSkeleton.getBestWeights());
			while(!s.hasLost()) {
				s.makeMove(p.pickMove(s,s.legalMoves()));
			}
			System.out.println("Rows Cleared: " + s.getRowsCleared());
			if (s.getRowsCleared() < min) // Update new minimum seed
			{
	...
	{
		// Call functions to evaluate the features
		checkHeights();
		checkHoles();
		checkWells();
		checkHTransition();
